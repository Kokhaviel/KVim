...
class BlockWriter {
	private final byte blockType, keyType;
	private final List<Entry> entries;
	private final int blockLimitBytes, restartInterval;

	private int entriesSumBytes, restartCnt;
	...
	void mustAdd(Entry entry) throws BlockSizeTooSmallException {
		if(!tryAdd(entry, true)) {
			throw new BlockSizeTooSmallException();
		}
	}
	...
	private boolean tryAdd(Entry entry, boolean tryRestart) {
		...
		if(!restart) {
			...
			if(prefixLen <= 5 && keyType == REF_BLOCK_TYPE) {
				restart = true;
				prefixLen = 0;
			} else if(prefixLen == 0) {
				restart = true;
	...
	static class LogEntry extends Entry {
		final ObjectId oldId, newId;
		final long timeSecs;
		final short tz;
		final byte[] name, email, msg;
		...
		int valueSize() {
			return 2 * OBJECT_ID_LENGTH
					+ computeVarintSize(name.length) + name.length + computeVarintSize(email.length) + email.length
					+ computeVarintSize(timeSecs) + 2 + computeVarintSize(msg.length) + msg.length;
		}
