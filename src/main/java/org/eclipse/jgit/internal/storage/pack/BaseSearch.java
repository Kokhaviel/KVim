...
class BaseSearch {
	...
	void addBase(int objectType, byte[] pathBuf, int pathLen, int pathHash)
		...
		if (tailMode == 0) return;

		if (!alreadyProcessed.add(pathHash)) return;
	...
	private byte[] readTree(AnyObjectId id) throws IOException {
		TreeWithData tree = treeCache.get(id);
		if (tree != null) return tree.buf;
